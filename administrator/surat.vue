        ...
        <div class="col-sm-12 card-header bg-success text-white">
          <div class="row">
            ...
            <div class="col-lg-2 col-sm-4">
              ...
              type="button" size="sm" v-b-modal.modal_surat style="float:right">
          ...
          <ul class="list-group">
            <li class="list-group-item mb-2" v-for="item in surat">
              <b-row>
                ...
                <b-col>
                  <b-button variant="success" size="sm" v-on:click="previewFile(item)"
                  v-b-modal.modal_preview block>
                      <span class="fa fa-eye"></span> View File
                  </b-button>
                  <b-button variant="info" size="sm" v-on:click="Edit(item)"
                  v-b-modal.modal_surat block>
                      <span class="fa fa-edit"></span> Edit
                  </b-button>
                  <b-button variant="danger" size="sm"
                  v-on:click="Drop(item.id_surat)" block>
  ...
  module.exports = {
    ...
    methods: {
      ...
      get_surat : function(){
        ...
        if(this.find === ""){
          axios.get(base_url+"/surat/"+this.pagination.perPage+"/"+offset, conf)
          .then(response => {
          ...
          axios.post(base_url+"/surat/find/"+this.pagination.perPage+"/"+offset, form, conf)
          .then(response => {
            this.$bvToast.hide("loadingToast");
            this.surat = response.data.surat;
            this.pagination.rows = response.data.count;
